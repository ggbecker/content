...
# Step 2: Generate unified NIST control file
log_section "Step 2: Generating NIST 800-53 Control File"
log_info "Building unified control file from OSCAL + CIS mappings..."
if python3 sync_nist.py --verbose; then
    log_success "Control file generated (without guards)"
    TEMP_CONTROL="$REPO_ROOT/controls/nist_800_53.yml.no_guards"
    cp "$REPO_ROOT/controls/nist_800_53.yml" "$TEMP_CONTROL"
else
    log_error "Failed to generate control file"
    exit 1
fi
...
# Step 4: Apply product family guards
log_section "Step 4: Applying Product Family Guards"
log_info "Adding Jinja2 guards for product-specific rules/variables..."
if python3 generate_product_family_guards.py --target rhel --control-file "$REPO_ROOT/controls/nist_800_53.yml"; then
    log_success "Product family guards applied"
    log_info "Control file: controls/nist_800_53.yml"

    # Show guard statistics
    GUARD_COUNT=$(grep -c '{{% if' "$REPO_ROOT/controls/nist_800_53.yml" || echo "0")
    log_info "Guards added: $GUARD_COUNT"
else
    log_error "Failed to apply product guards"
    exit 1
fi
...
if [ "$SKIP_COMPARISON" = false ]; then
    ...
    for product in $PRODUCTS; do
        ...
        # Check if both subtractions produce empty profiles
        DIFF_A_EMPTY=false
        DIFF_B_EMPTY=false
        
        if echo "$DIFF_A_OUTPUT" | grep -q "Subtraction would produce an empty profile"; then
            DIFF_A_EMPTY=true
            echo "  Debug: CIS - CIS_NIST = empty ✓"
        else
            echo "  Debug: CIS - CIS_NIST has differences"
        ...
        if echo "$DIFF_B_OUTPUT" | grep -q "Subtraction would produce an empty profile"; then
            DIFF_B_EMPTY=true
            echo "  Debug: CIS_NIST - CIS = empty ✓"
        else
            echo "  Debug: CIS_NIST - CIS has differences"
            echo "  Debug: First 10 lines of output:"
            echo "$DIFF_B_OUTPUT" | head -10
        fi

        if [ "$DIFF_A_EMPTY" = true ] && [ "$DIFF_B_EMPTY" = true ]; then
            log_success "$product: Profiles match exactly ✓"
        else
            log_error "$product: Profiles differ!"
            
            if [ "$DIFF_A_EMPTY" = false ]; then
                echo ""
                echo "  Rules in CIS but not in CIS_NIST:"
                echo "$DIFF_A_OUTPUT" | head -20
            fi
            
            if [ "$DIFF_B_EMPTY" = false ]; then
                echo ""
                echo "  Rules in CIS_NIST but not in CIS:"
                echo "$DIFF_B_OUTPUT" | head -20
            fi
...
echo ""
log_info "Next steps:"
echo "  - Review changes: git diff controls/nist_800_53.yml"
echo "  - Review profiles: git diff products/*/profiles/cis_nist.profile"
echo "  - Commit changes: git add controls/ products/ utils/nist_sync/data/"
echo ""
