...
Examples:
  # Target all RHEL versions
  %(prog)s --target rhel --control-file controls/nist_800_53.yml

  # Target RHEL and OCP families
  %(prog)s --target rhel ocp --control-file controls/nist_800_53.yml

  # Target specific products
  %(prog)s --target rhel8 rhel9 ocp4 --control-file controls/nist_800_53.yml
...
Output example with family guards:
  ...
  {{{% endif %}}
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help='Path to control file to process'
    )
    parser.add_argument(
        '--output',
        type=Path,
