...
class NISTControlSync:
    ...
    def load_control_file(self) -> Dict:
        """Load existing control YAML file, or create skeleton if it doesn't exist."""
        if self.control_file.exists():
            ...
            print(f"Creating new control file at {self.control_file}")
            data = {}

        # Ensure required top-level keys exist
        if 'policy' not in data:
            data['policy'] = 'NIST 800-53 Revision 5'
        if 'title' not in data:
            data['title'] = 'NIST Special Publication 800-53 Revision 5'
        if 'id' not in data:
            data['id'] = 'nist_800_53'
    ...
    def sync_controls(self, verbose: bool = False) -> bool:
        ...
        for oscal_ctrl in oscal_controls:
            ...
            if ctrl_id in existing_controls:
                # UPDATE: Update metadata and levels, preserve rules and status
    ...
    def save_control_file(self, data: Dict):
        """Atomically save control file with preserved formatting."""
