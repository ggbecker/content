...
# Installs an auditd.service dropin to restore 0600 on audit.rules and auditd.conf after they
# are reset to 0640 on auditd start (augenrules rewrites audit.rules when /etc/audit/rules.d/
# content changes; auditd itself resets auditd.conf). RHEL 8/9 only, not containers.
...
# Generate the dropin file content to restore 0600 on audit.rules and auditd.conf after they are reset to 0640 on auditd start (augenrules rewrites audit.rules when /etc/audit/rules.d/ content changes; auditd itself resets auditd.conf). RHEL 8/9 only, not containers.
...
[Service]
ExecStartPost=/usr/bin/chmod 0600 /etc/audit/audit.rules /etc/audit/auditd.conf
EOF
