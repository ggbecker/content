...
def extract_rule_id_from_definition(def_id: str) -> str:
    """
    Extract the rule ID from an OVAL definition ID.

    Examples:
        "oval:ssg-accounts_password_pam_dcredit:def:1" -> "accounts_password_pam_dcredit"
        "accounts_password_pam_dcredit" -> "accounts_password_pam_dcredit"

    Args:
        def_id: The OVAL definition ID

    Returns:
        The extracted rule ID
    """
    if ':' in def_id:
        # Format: oval:ssg-rule_id:def:1
        parts = def_id.split(':')
        if len(parts) >= 2 and parts[1].startswith('ssg-'):
            return parts[1][4:]  # Remove 'ssg-' prefix
    # Already in simple format
    return def_id
...
def process_oval_file(oval_file: Path) -> Dict[str, Set[str]]:
    ...
    Returns:
        Dictionary mapping rule IDs to sets of variable IDs
    """
    rule_vars = {}

    try:
        # Read the content as string for regex matching
        with open(oval_file, 'r', encoding='utf-8') as f:
            content = f.read()

        # Extract definition IDs directly from content using regex
        # This is more reliable than XML parsing for files with namespace issues
        # Format: <definition ... id="oval:ssg-rule_id:def:1" or id="rule_id"
        def_id_pattern = r'<definition[^>]+id="([^"]+)"'
        def_matches = re.findall(def_id_pattern, content)

        # For each definition, extract the rule ID and find variables
        for def_id in def_matches:
            rule_id = extract_rule_id_from_definition(def_id)

            # Find all var_ref attributes in the content
            var_refs = extract_variables_from_oval_content(content)

            if var_refs:
                if rule_id not in rule_vars:
                    rule_vars[rule_id] = set()
                rule_vars[rule_id].update(var_refs)
